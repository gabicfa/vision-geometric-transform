...
Kmo, Rmo, Tmo = cam2.factor()
print("Camera OBJETIVO")
#print(Kmo)

#print("Rotation")
#print(Rm)
phi, theta, psi = mat2euler(Rmo)
#print("Rotation OBJETIVO: {0:.2f} , {1:.2f}, {2:.2f}".format(math.degrees(phi), math.degrees(theta), math.degrees(psi)))

#print("Translation OBJETIVO")
#print(Tmo)
Oz = Tmo[0]
Oy = Tmo[1]
Ox = Tmo[2]
...
while(True):
    ...
    # frameWidth = int(img1bgr.get(cv2.cv.CV_CAP_PROP_FRAME_WIDTH))
    # frameHeight = int(img1bgr.get(cv2.cv.CV_CAP_PROP_FRAME_HEIGHT))

    #cv2.putText(img1bgr, "texto", (int(frameWidth/2 - 20),int(frameHeight/2)),  cv2.FONT_HERSHEY_DUPLEX, 3, (255,0,0),thickness = 2, lineType = cv2.CV_AA)





    if Py > Oy+margem and Pz > Oz+margem and Px > Ox+margem and Py < Oy-margem and Pz < Oz-margem and Px < Ox-margem:
        cv2.putText(img1bgr, "Chegou no objetivo", (290,350),  cv2.FONT_HERSHEY_DUPLEX, 1, (255,0,0),thickness = 2, lineType = cv2.CV_AA)

        print "Voce chegou no seu objetivo"

    else:
        if Pz > Oz+margem:
            cv2.putText(img1bgr, "Va para frente", (290,350),  cv2.FONT_HERSHEY_DUPLEX, 1, (255,0,0),thickness = 2, lineType = cv2.CV_AA)

        if Py > Oy+margem:
            cv2.putText(img1bgr, "Va para cima", (290,50),  cv2.FONT_HERSHEY_DUPLEX, 1, (255,0,0),thickness = 2, lineType = cv2.CV_AA)
            print "Va para cima"

        if Px > Ox+margem:
            cv2.putText(img1bgr, "Va para a esquerda", (10,300),  cv2.FONT_HERSHEY_DUPLEX, 1, (255,0,0),thickness = 2, lineType = cv2.CV_AA)
            print"Va para a esquerda"


        if Pz < Oz-margem:
            cv2.putText(img1bgr, "Va para tras", (290,350),  cv2.FONT_HERSHEY_DUPLEX, 1, (255,0,0),thickness = 2, lineType = cv2.CV_AA)
            print "Va para tras"

        if Py < Oy-margem:
            cv2.putText(img1bgr, "Va para baixo", (290,550),  cv2.FONT_HERSHEY_DUPLEX, 1, (255,0,0),thickness = 2, lineType = cv2.CV_AA)
            print "Va para baixo"

        if Px < Ox-margem:
            cv2.putText(img1bgr, "Va para a direta", (520,300),  cv2.FONT_HERSHEY_DUPLEX, 1, (255,0,0),thickness = 2, lineType = cv2.CV_AA)
            print "Va para direita"
